...
runs = parameters("/home/nerve/Downloads/settlingdata")
runs.files()
...
for item in param:
    ...
    savepath = '../test/'+item.split('/')[-2]
    outfile = item[:-16]+'test.out'
    ...
    for iteration in list(range(0, n)):
        ...
        ax.legend(loc='best')
        fig.tight_layout()
        fig.savefig('../test/slopemethod_'+savename)
        ...
        ax.legend(loc='best')
        fig.tight_layout()
        fig.savefig('../test/pmethod_'+savename)
        ...
        ax.legend(loc='best')
        fig.tight_layout()
        fig.savefig('../test/data_'+savename)
        pl.close('all')
